...
- <a href="./concurrency/processor.go#L12"><img align="center" src="https://capsule-render.vercel.app/api?type=soft&color=6699ff&fontColor=ffffff&height=200&section=header&text=Processor&fontSize=100&animation=fadeIn&fontAlignY=55" width="100" height="23"/></a> is the main unit of work for doing distributed stream processing. It uses golang concurrent capabilities in order to maximize and optimize the performace of the outcome. In most of the cases a `Processor` acts as an enricher of the data stream and it can be combine within a `MsgMultiplexer` with other processors to produce complex deterministic outputs. `Processor`has a `Process` function that encapsulates and decouples the processing functionality and with the help of `ProcessorTransformFn` we can taylor output as required making it highly configurable and versatile. 
